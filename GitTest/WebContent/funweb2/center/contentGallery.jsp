...
bb = bdao.getGallery(bb.getNum());
bb.setReadcount(bb.getReadcount() + 1);
bdao.updateGallery(bb);
...
<script type="text/javascript">
 ...
 function deleteContent() {
	location.href="contentDeleteForm.bo?num=<%=bb.getNum()%>&type=1";
}
 
 function deleteComment() {
		location.href="commentDeletePro.bo?num"
 }
 
 function goToNotice() {
	location.href="gallery.bo<%=sb.toString() %>";
}
 
 function replyContent() {
	location.href="contentReplyForm.bo?type=1";
}
...
<article>
<div>
<form action="contentGalleryUpdateForm.bo" method="post" id="write">
...
<%if(bb.getFile() != null){ %>
<tr><td align="center">파일</td><td><a href="../../upload/<%=bb.getFile()%>"><%=bb.getFile() %></a><img src="../../upload/<%=bb.getFile()%>"/></td></tr> 
<tr><td align="center">파일 다운</td><td><a href="file_down.jsp?file_name=<%=bb.getFile()%>">다운로드</a></td></tr> 

<%} %>
<tr><td align="center">내용</td>
    <td><textarea rows="20" cols="70" name="content" readonly="readonly"><%=bb.getContent() %></textarea></td></tr>
...
if (id.equals(bb.getName())){
	// 뉴스값 수정
	bdao.updateGallery(bb.getNum(), 0);
	//
	
	%>
<input type="submit" value="수정" class="submit"><input type="button" value="삭제" onclick="deleteContent()">
<%} %>
<input type="button" value="답글" onclick="replyContent()">
<%} %>
<input type="button" value="목록" onclick="goToNotice()"></div></td></tr>
</table>
</form>
</div>
</article>
...
%>
<tr><td align="center">글쓴이</td><td><%=cb.getName() %></td>
<tr><td align="center">날짜</td><td><%=dateStr %></td></tr>
<tr><td align="center">내용</td>
    <td><textarea rows="5" cols="70" name="content" readonly="readonly"><%=cb.getContent() %></textarea></td></tr>
<tr><td colspan="2" align="right">
<div id="buttons">
<%
if(id != null){
...
<a href="commentDeletePro.bo?ref=<%=cb.getRep_ref()%>&num=<%=cb.getNum()%>&type=1"><input type="button" value="삭제"></a>
<%} %>
...
<article>
<div>
<form action="contentGalleryCommentPro.bo" method="post" id="write">
...
<input type="button" value="취소" onclick="deleteContent()">
</div></td></tr>
</table>
</form>
</div>

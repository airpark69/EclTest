...
ArrayList pageGalleryList = (ArrayList)bdao.getGalleryList(startRow, pageSize);

pageGalleryList = (ArrayList)bdao.getGalleryList(listOpt);
int count = pageGalleryList.size();
if(opt == null){
	count = bdao.getGalleryCount();
}
%>
...
for(int x = 0; x < pageGalleryList.size(); x++){
	bb = (BoardBean)pageGalleryList.get(x);
	%>
	<tr><td><%=bb.getNum() %></td><td class="left"><a href="contentGallery.bo?num=<%=bb.getNum()%>&pageNum=<%=currentPage %>">
...
<%if(id != null) {%>
<div id="table_search">
<a href="writeForm.bo?type=1"><input type="button" value="글쓰기"></a>
</div>
<%} %>
...
<a href="gallery.bo?pageNum=<%=currentPage - 1%>">Prev</a>
...
for(int x = 0; x < lastPage;x++){
	%><a href="gallery.bo?pageNum=<%=x+1%>"><%=x+1 %></a>
...
<a href="gallery.bo?pageNum=<%=currentPage + 1%>">Next</a>
